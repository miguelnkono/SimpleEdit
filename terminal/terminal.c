...
int getWindowSize(int *rows, int *cols)
{
  ...
  {
    if (write(STDOUT_FILENO, CURSOR_BOTTOM_RIGHT, CURSOR_BOTTOM_RIGHT_SIZE) != 12)
      return -1;
...
int getCursorPosition(int *rows, int *cols)
{
  ...
  if (write(STDOUT_FILENO, CURSOR_POSITION, CURSOR_POSITION_SIZE) != 4)
    return -1;
  ...
  if (buf[0] != ESCAPE_SEQUENCE || buf[1] != '[')
    return -1;
